...
impl Display for Record {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "{} {} {} {}",
            self.name, self.ttl, self.class, self.rrtype
        )?;
...
impl<'a> ZoneParser<'a> {
    ...
    pub fn rrtype_from_str(&self, rrtype_str: &str) -> Result<RRType, String> {
        ...
        if let Some(rrtype) = self.rrtype_hash.get(&lcstr) {
            ...
            let rrtype = RRType::from_discriminant(
                lcstr[4..]
                    .parse()
                    .expect(&format!("Unknown type {}", rrtype_str)),
            );
            return Ok(rrtype);
        } else {
    ...
    pub fn rrtype_bm_from_str(&self, rrtype_str: &str) -> Result<(u8, u128, u128), String> {
        let lcstr = rrtype_str.to_lowercase();

        if let Some(bm) = self.rrtype_bm_hash.get(&lcstr) {
            return Ok(*bm);
        } else if lcstr.starts_with("type") {
            let t_disc: u16 = lcstr[4..]
                .parse()
                .expect(&format!("Unknown type {}", rrtype_str));
            let window_block = (t_disc >> 8) as u8;
            let bitpos = t_disc & 0xff;
    ...
    fn unescape_quoted_data(&mut self, part: &[u8]) -> Result<bool, String> {
        ...
        for p in part.split_inclusive(|&b| b == b'\\' || b == b'"') {
            ...
            if esc_end {
                ...
                if plen >= 3 {
                    if OCTAL1.contains(&p[0]) && OCTAL2.contains(&p[1]) && OCTAL2.contains(&p[2]) {
                        let i = 64 * (p[0] - b'0') + 8 * (p[1] - b'0') + (p[2] - b'0');
                ...
                if p[plen - 1] == b'"' {
                    ...
                    self.quoted_buf
                        .push_str(&p[remaining..plen - 1].escape_bytes().to_string());
                    quote_end = true;
                    esc_end = false;
                    continue;
                }
                ...
                //   push part
                self.quoted_buf
                    .push_str(&p[remaining..plen].escape_bytes().to_string());
                esc_end = false;
                continue;
            }
            ...
            if p[plen - 1] == b'"' {
                // Part with end quote
                self.quoted_buf
                    .push_str(&p[0..plen - 1].escape_bytes().to_string());
                quote_end = true;
                continue;
            }
            ...
            match self.state {
                ...
                ParserState::Common => {
                    ...
                    if let Some(class) = self.rrclass_hash.get(&word) {
                        ...
                        // Expect TTL
                        self.ttl = word.parse().or(Err(format!(
                            "Unexpected content on line {} pos {}",
                            self.line_no, pos
                        )))?;
                    }
                }
                ParserState::Directive => {
                    ...
                    if self.directive_buf == "$ttl" {
                        self.default_ttl = value.parse().or(Err(format!(
                            "Unexpected content on line {} pos {}",
                            self.line_no, pos
                        )))?;
                        ...
                        return Err(format!("Unknown directive {}", self.directive_buf));
                    }
                    self.state = ParserState::Init;
                }
                ParserState::Data => {
                    if part[0] == b'"' {
                        ...
                        if end_quote {
                            ...
                            rec.as_mut()
                                .unwrap()
                                .push_data(RecordData::new(&self.quoted_buf));
                        } else {
                        ...
                        // Unquoted data
                        self.quoted_buf.clear();
                        let end_quote = self.unescape_quoted_data(&part[0..wlen])?;
                        if end_quote {
                            self.quoted_buf.push('"');
                        }
                        rec.as_mut()
...
#[cfg(test)]
mod tests {
    use crate::zoneparser::{RRClass, RRType, Record, RecordData, ZoneParser};
    use std::fs::File;
    ...
    impl Record {
        pub fn new_with_data(
            ...
            let recorddata = data.iter().map(|s| RecordData::new(s)).collect::<Vec<_>>();
